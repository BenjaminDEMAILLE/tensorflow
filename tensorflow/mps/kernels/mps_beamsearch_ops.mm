...
// Beam search operations for MPS backend
...
#define BEAM_OP(name) \
  ...
  extern "C" void MPS##name##_Compute(void* kernel, TF_OpKernelContext* ctx) { \
    ...
    TF_SetStatus(status, TF_UNIMPLEMENTED, #name " is CPU-only"); \
    TF_OpKernelContext_Failure(ctx, status); \
    TF_DeleteStatus(status); \
  }

BEAM_OP(BeamSearchDecoder)
BEAM_OP(BeamSearchDecoderV2)
BEAM_OP(GatherTree)
BEAM_OP(TopKWithUnique)
